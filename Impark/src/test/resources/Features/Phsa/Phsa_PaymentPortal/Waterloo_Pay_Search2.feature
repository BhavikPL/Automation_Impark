...
When user click on to the "Void" link of "search_number" citation number ticket from payment application
And user wait for "5" seconds
Then verify cancel citation form with "search_number" key as citation number
When user select cancel citation reason and save its value as "search_cancel_reason" into temp file from payment application
And user wait for "2" seconds
When user get cancel citation comments and save its value as "search_cancel_comment" into temp file from payment application
And user wait for "2" seconds
...
Then verify that notice with "search2_number" citation number has been searched successfully from payment application
Then verify that notice with "search_plate" citation plate has been searched successfully from payment application
...
And user wait for "1" seconds
When user click on to the "Pay" button from top bar from payment application
And user wait for "5" seconds
Then verify that page has "Plate number" label and "search_plate" as key its value from payment application
Then verify that page has "Notice number" label and "search2_number" as key its value from payment application
Then verify that page has "Notice number" label and "search2_number" as key its value from payment application
Then verify that page has "Payment amount" label and "search2_fine" as key its value from payment application
Then verify that Total Amount is the summation of "search2_fine" from make payment screen from payment application
